...
data_am = data_am %>%
  rename(
    PARITY = PARITYAM, 
    AM_YIELD = MILK_YIELDAM, 
    SCC = SCCAM, 
    HERD = HERDAM, 
    ACT_Fat = ACT_FATAM, 
    ACT_Pro = ACT_PROAM, 
    MILK_YIELD = TOTAL_MILK_YIELD
  ) %>%
  mutate(
...
for (subdivision in subdivisions) {
  ...
  if (nrow(train_subset) > 0 && nrow(test_subset) > 0) {
    ...
    for (i in 1:k) {
      ...
      model_milk = lmer(MILK_YIELD ~ AM_YIELD + ACT_Fat + ACT_Pro + MILK_INTERVAL + (1 | Herd_yr), data = fold_train_subset)
      model_fat = lmer(Total_fat ~ AM_YIELD + ACT_Fat + ACT_Pro + MILK_INTERVAL + (1 | Herd_yr), data = fold_train_subset)
      model_pro = lmer(Total_Pro ~ AM_YIELD + ACT_Fat + ACT_Pro + MILK_INTERVAL + (1 | Herd_yr), data = fold_train_subset)
      

      fold_train_subset$PREDICTED_TOTAL_MILK_YIELD = predict(model_milk, newdata = fold_train_subset, allow.new.levels = TRUE)
      fold_train_subset$PREDICTED_TOTAL_FAT_PCT = predict(model_fat, newdata = fold_train_subset, allow.new.levels = TRUE)
      fold_train_subset$PREDICTED_TOTAL_PRO_PCT = predict(model_pro, newdata = fold_train_subset, allow.new.levels = TRUE)
      
      fold_test_subset$PREDICTED_TOTAL_MILK_YIELD = predict(model_milk, newdata = fold_test_subset, allow.new.levels = TRUE)
      fold_test_subset$PREDICTED_TOTAL_FAT_PCT = predict(model_fat, newdata = fold_test_subset, allow.new.levels = TRUE)
      fold_test_subset$PREDICTED_TOTAL_PRO_PCT = predict(model_pro, newdata = fold_test_subset, allow.new.levels = TRUE)
      ...
      train_mape_pro_values <- c(train_mape_pro_values, mape(fold_train_subset$ACT_Pro, fold_train_subset$PREDICTED_TOTAL_PRO_PCT))
      test_mape_pro_values <- c(test_mape_pro_values, mape(fold_test_subset$ACT_Pro, fold_test_subset$PREDICTED_TOTAL_PRO_PCT))
      train_wmape_pro_values <- c(train_wmape_pro_values, wmape(fold_train_subset$ACT_Pro, fold_train_subset$PREDICTED_TOTAL_PRO_PCT))
      test_wmape_pro_values <- c(test_wmape_pro_values, wmape(fold_test_subset$ACT_Pro, fold_test_subset$PREDICTED_TOTAL_PRO_PCT))
      
  
      train_mape_fat_values = c(train_mape_fat_values, mape(fold_train_subset$ACT_Fat, fold_train_subset$PREDICTED_TOTAL_FAT_PCT))
      test_mape_fat_values = c(test_mape_fat_values, mape(fold_test_subset$ACT_Fat, fold_test_subset$PREDICTED_TOTAL_FAT_PCT))
      train_wmape_fat_values = c(train_wmape_fat_values, wmape(fold_train_subset$ACT_Fat, fold_train_subset$PREDICTED_TOTAL_FAT_PCT))
      test_wmape_fat_values = c(test_wmape_fat_values, wmape(fold_test_subset$ACT_Fat, fold_test_subset$PREDICTED_TOTAL_FAT_PCT))
    }
    ...
    full_model_milk = lmer(MILK_YIELD ~ AM_YIELD+ ACT_Fat + ACT_Pro+ MILK_INTERVAL + (1 | Herd_yr), data = train_subset)
    full_model_fat = lmer(Total_fat ~ AM_YIELD + ACT_Fat + ACT_Pro+ MILK_INTERVAL + (1 | Herd_yr), data = train_subset)
    full_model_pro = lmer(Total_Pro ~ AM_YIELD + ACT_Fat + ACT_Pro+ MILK_INTERVAL + (1 | Herd_yr), data = train_subset)
    

    test_subset$PREDICTED_TOTAL_MILK_YIELD = predict(full_model_milk, newdata = test_subset, allow.new.levels = TRUE)
    test_subset$PREDICTED_TOTAL_FAT_PCT = predict(full_model_fat, newdata = test_subset, allow.new.levels = TRUE)
    test_subset$PREDICTED_TOTAL_PRO_PCT = predict(full_model_pro, newdata = test_subset, allow.new.levels = TRUE)
    ...
    test_mape_fat = mape(test_subset$ACT_Fat, test_subset$PREDICTED_TOTAL_FAT_PCT)
    test_wmape_fat = wmape(test_subset$ACT_Fat, test_subset$PREDICTED_TOTAL_FAT_PCT)
    
    test_mape_pro = mape(test_subset$ACT_Pro, test_subset$PREDICTED_TOTAL_PRO_PCT)
    test_wmape_pro = wmape(test_subset$ACT_Pro, test_subset$PREDICTED_TOTAL_PRO_PCT)
...
for (subdivision in subdivisions) {
  ...
  if (nrow(train_subset) > 0 && nrow(test_subset) > 0) {
    ...
    for (i in 1:k) {
      ...
      # Fit mixed-effects models
      model_milk = lmer(MILK_YIELD ~ PM_YIELD + PM_FAT + PM_PRO + MILK_INTERVAL + (1 | Herd_yr), data = fold_train_subset)
      model_fat = lmer(Total_fat ~ PM_YIELD + PM_FAT + PM_PRO + MILK_INTERVAL + (1 | Herd_yr), data = fold_train_subset)
      model_pro = lmer(Total_Pro~ PM_YIELD + PM_FAT + PM_PRO + MILK_INTERVAL + (1 | Herd_yr), data = fold_train_subset)
      ...
      # Calculate MAPE and WMAPE for protein percentage
      train_mape_pro_values = c(train_mape_pro_values, mape(fold_train_subset$PM_PRO, fold_test_subset$PREDICTED_TOTAL_PRO_PCT))
      test_mape_pro_values = c(test_mape_pro_values, mape(fold_test_subset$PM_PRO, fold_test_subset$PREDICTED_TOTAL_PRO_PCT))
      train_wmape_pro_values = c(train_wmape_pro_values, wmape(fold_train_subset$PM_PRO, fold_test_subset$PREDICTED_TOTAL_PRO_PCT))
      test_wmape_pro_values = c(test_wmape_pro_values, wmape(fold_test_subset$PM_PRO, fold_test_subset$PREDICTED_TOTAL_PRO_PCT))
      
      # Calculate MAPE and WMAPE for fat percentage
      train_mape_fat_values = c(train_mape_fat_values, mape(fold_train_subset$PM_FAT, fold_test_subset$PREDICTED_TOTAL_FAT_PCT))
      test_mape_fat_values = c(test_mape_fat_values, mape(fold_test_subset$PM_FAT, fold_test_subset$PREDICTED_TOTAL_FAT_PCT))
      train_wmape_fat_values = c(train_wmape_fat_values, wmape(fold_train_subset$PM_FAT, fold_test_subset$PREDICTED_TOTAL_FAT_PCT))
      test_wmape_fat_values = c(test_wmape_fat_values, wmape(fold_test_subset$PM_FAT, fold_test_subset$PREDICTED_TOTAL_FAT_PCT))
    }
    # Store the average results of CV
...
for (subdivision in subdivisions) {
  ...
  if (nrow(train_subset) > 0 && nrow(test_subset) > 0) {
    ...
    model_milk = lmer(MILK_YIELD ~ PM_YIELD + PM_FAT + PM_PRO + MILK_INTERVAL + (1 | Herd_yr), data = train_subset)
    model_fat = lmer(Total_fat ~ PM_YIELD + PM_FAT + PM_PRO + MILK_INTERVAL + (1 | Herd_yr), data = train_subset)
    model_pro = lmer(Total_Pro ~ PM_YIELD + PM_FAT + PM_PRO + MILK_INTERVAL + (1 | Herd_yr), data = train_subset)
    
  
    test_subset$PREDICTED_TOTAL_MILK_YIELD = predict(model_milk, newdata = test_subset, allow.new.levels = TRUE)
    test_subset$PREDICTED_TOTAL_FAT_PCT = predict(model_fat, newdata = test_subset, allow.new.levels = TRUE)
    test_subset$PREDICTED_TOTAL_PRO_PCT = predict(model_pro, newdata = test_subset, allow.new.levels = TRUE)
    
    
    test_mape_milk = mape(test_subset$MILK_YIELD, test_subset$PREDICTED_TOTAL_MILK_YIELD)
    test_wmape_milk = wmape(test_subset$MILK_YIELD, test_subset$PREDICTED_TOTAL_MILK_YIELD)
    
    test_mape_fat = mape(test_subset$PM_FAT, test_subset$PREDICTED_TOTAL_FAT_PCT)
    test_wmape_fat = wmape(test_subset$PM_FAT, test_subset$PREDICTED_TOTAL_FAT_PCT)
    
    test_mape_pro = mape(test_subset$PM_PRO, test_subset$PREDICTED_TOTAL_PRO_PCT)
    test_wmape_pro = wmape(test_subset$PM_PRO, test_subset$PREDICTED_TOTAL_PRO_PCT)
